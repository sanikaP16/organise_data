const people = [
  {
    ...
    hobbies: ["Playing chess", "gardening", "cycling"],
    hasCar: true,
    pets: [
      {
        name: "Max",
    ...
    pets: [
      {
        ...
        typeOfPet: "Parrot",
        age: 2,
        favoriteActivity: "mimic Ananya's voice",
        isVaccinated: true,
    ...
    pets: [
      {
        ...
        typeOfPet: "Cat",
        breed: "Parsian cat",
        favoriteActivity: "launging in the sun",
        isVaccinated: false,
        isFullyVaccinated: true,
      },
      {
    ...
    pets: [
      {
        ...
        isVaccinated: true,
        isFullyVaccinated: false,
        favoriteActivity: ["hopping aroung backyard", "nibbling on carrots"]
      }
    ]
  }
];
...
// 14. What is the most common type of pet among the group? ///////// need to change
...
// 16. How many individuals share at least one hobby with Ramesh? /// review again
...
const getMinAge = (minimumAge, { age }) => Math.min(minimumAge, age);
...
const peopleWitoutPets = (people) => {
  return people.filter(({ pets }) => pets.length === 1)
    .map(({ name }) => name);
}
